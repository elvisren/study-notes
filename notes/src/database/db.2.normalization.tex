...
\begin{theorem}
    A decomposition of $R$ into $\set{R_i}$ is \cindex{dependency preserving} if and only if $\left (\bigcup_i F_i \right)^{+} = F^{+}$
\end{theorem}
...
\begin{definition}[Join Dependency]
    A relational schema is \cindex{join dependency} $JD(R_1, R_2, \dots, R_n)$ if for every legal relation $r(R)$, we have $\left(\pi_{R_1} (r), \pi_{R_2}(r), \dots, \pi_{R_n}(r) \right) = r$.
\end{definition}
