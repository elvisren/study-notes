...
\begin{example}[closure of path connected may not be path connected]
    The \cindex{topologist's sine curve} $\closure{S} = \closure{\set{x \times \sin \left(\frac{1}{x} \right) | 0 < x \leq 1}}$ is connected but not path connected. So if $X$ is path connected, $\closure{X}$ may not be path connected.
