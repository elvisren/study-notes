...
\end{itemize}
    \item The sequence lemma (\theoref{sequence_lemma}) is quite useful in proving a topology space is not metrizable. A sketch of the proof is:
    \begin{itemize}
...
\begin{definition}[\cindex{K-topology}]
...
\begin{definition}[\cindex{$S_\Omega$}]
...
\begin{definition}[\cindex{$I_o^2$}]
    ...
    Let $I=[0,1]$. The dictionary order on $I \times I$ is called \cindex{ordered square}, and denoted as $I_o^2$. 
    
    The subspace topology of $I \times I$ inherited from dictionary order topology of $\realnumber \times \realnumber$ is different from $I_o^2$. For example, $0.5 \times (0.5,1]$ is open as subspace topology, but not for $I_o^2$. This is because $I \times I$ is not convex. See \theoref{convex_order_topology_are_the_same} for a proof.
\end{definition}
...
\begin{definition}[\cindex{topologist's sine curve}]\label{topologist_sine_curve}
...
\begin{definition}[\cindex{interior point}]
...
\begin{definition}[\cindex{diameter}]
...
\begin{definition}[\cindex{euclidean metric}]
...
\begin{definition}[\cindex{square metric}]
...
\begin{definition}[\cindex{uniform metric}]
    ...
    The topology generated by uniform metric is called \cindex{uniform topology}.
...
\end{theorem}
\begin{proof}
    For a $A \subset X$,  $x_n \rightarrow x \Rightarrow x \in \closure{A}$. $f(x_n) \rightarrow f(x) \Rightarrow f(x) \in \closure{f(A)}$. Use \defiref{first_countability_axiom}, for every $x \in \closure{A}$, we could find $x_i \rightarrow x$.
\end{proof}




\begin{definition}[\cindex{converge uniformly}]\label{converge_uniformly}
...
\begin{example}[$\realnumber^\omega$ is not metrizable in box topology]
    Let $A = \set{(x_1, x_2, \cdots ) | x_i > 0}$. We have $(0,0,0, \cdots) \in \closure{A}$. For a sequence $a_n = (x_{n_1}, x_{n_2}, \cdots)$, the open set $\set{(- x_{1_1}, x_{1_1}) \times (- x_{2_2}, x_{2_2}) \times \cdots}$ contains no $a_i$. Use \theoref{sequence_lemma} to prove that it is not metrizable.
\end{example}


\begin{example}[$\realnumber^J$ is not metrizable in product topology]
...
\begin{definition}[\cindex{quotient topology}]
