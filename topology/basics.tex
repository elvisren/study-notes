...
\begin{itemize}
    ...
    \item \cindex{lower limit topology} \cindex{$\realnumber_l$}: $[a,b) = \set{x | a \leq x < b}$.
\end{itemize}
\end{example}
...
\begin{definition}[\cindex{separable}]\label{separable}
    A topological space is separable\footnote{Separable is different from separation (see \defiref{separation}).} if it has a \emph{countable} dense subset.
\end{definition}
...
\begin{definition}[\cindex{limit point}]
    ...
    It is also called \cindex{cluster point}.
\end{definition}
...
\begin{theorem}[\emph{$\realnumber^{\omega}$ is metrizable}]
...
\begin{example}[\emph{$\realnumber^\omega$ is not metrizable in box topology}]
...
\begin{example}[\emph{$\realnumber^J$ is not metrizable in product topology}]
