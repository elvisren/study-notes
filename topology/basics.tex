...
\begin{definition}[\cindex{uniform metric}]
    ...
    An open ball of uniform metric is a spacial tube. Let $\set{x_i}$ be a point in $\realnumber^\omega$. For $0 < \epsilon < 1$, define a tube $U(x,\epsilon) = \prod_i (x_i - \epsilon, x_i + \epsilon)$. The open ball of $x$ with diameter $\epsilon$ is
    \begin{equation}
...
\begin{theorem}[\cindex{sequence lemma}]\label{sequence_lemma}
    ...
    Note: the sequence lemma is quite useful in \emph{proving a topology space is not metrizable}. A sketch of the proof is:
    \begin{itemize}
