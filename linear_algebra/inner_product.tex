...
\begin{definition}
    An \cindex{inner product} is a function $\innerproduct{x}{y}: V \rightarrow V \rightarrow F$ that:
\begin{itemize}
...
\begin{definition}[conjugate transpose]
    The \cindex{conjugate transpose} (\cindex{adjoint}) of $A_{m \times n}$ is 
    \begin{equation}
...
\begin{definition}[orthogonal]
    $x$ and $y$ are \cindex{orthogonal} if $\innerproduct{x}{y} = 0$. A subset $S$ of $V$ is orthogonal if any two vectors in $S$ are orthogonal. 
...
\begin{definition}[orthogonal complement]
    For a subset $S$ of $V$, the \cindex{orthogonal complement} $S^\perp$ is defined as $S^\perp=\set{x \in V: \forall y \in S, \innerproduct{x}{y} = 0}$.
\end{definition}
...
\begin{theorem}
    For an orthogonal subset $S=\set{v_1, v_2, ..., v_k}$ of $V$. Define the \cindex{projection} of $y$ on $v$ as
    \begin{equation}
...
\begin{theorem}[Riesz representation theorem]\label{eiesz_representation_theorem}
...
\begin{definition}[Hermitian, self-adjoint]
    $T$ is \cindex{Hermitian} (\cindex{self-adjoint}) if $T = T^*$. So is it for $A$.
\end{definition}
...
\begin{definition}[Unitarily Equivalent]    
    If $D$ and $A$ are similar, we have $D = \inverse{Q} A Q$. If, in addition, $\inverse{Q} = Q^*$, $D$ and $A$ are called \cindex{unitarily equivalent} (or \cindex{orthogonally equivalent}). Here we did not require $A$ to be unitary or orthogonal.
\end{definition}
...
\begin{definition}[Rigid Motion]
    Let $V$ be real inner product space. A function $f$ is called \cindex{rigid motion} if 
    \begin{equation}
        \norm{f(x) - f(y)} = \norm{x - y}
    \end{equation}
\end{definition}
...
\begin{definition}[Reflection]
    Let $L$ be a line in $R^2$ that passes through the original. $T$ is called \cindex{reflection} about $L$ if $T(x) = x$ for $x \in L$ and $T(x) = -x$ for $x \in L^\perp$. 
...
\begin{definition}[Translation]
    For a vector $v$, a \cindex{translation} $T$ is $T(x) = x + v$ for all $x$. Translation is a rigid motion.
\end{definition}
...
\begin{definition}[Projection]
    Let $V = W_1 \oplus W_2$. For $x = a + b$ where $a \in W_1$ and $b \in W_2$, if $T(x) = a$, then $T$ is a \cindex{projection} on $W_2$ along $W_1$.
\end{definition}
...
\begin{definition}[Positive Definite]
    A linear operator $T$ is \cindex{positive definite} if $T$ is self-adjoint and for $x \in V$,
    \begin{equation}
...
\begin{theorem}[$\sqrt{T}$]
   A positive semidefinite $T$ could have many \cindex{square root}, that is $BB = T$, but it only have one \cindex{positive semidefinite square root}. This square root is called \cindex{$\sqrt{T}$}. $\sqrt{T}$ has a property that
   \begin{equation}
...
\begin{definition}[Singular Value]
    $\set{v_i}$ might be different because there are multiple choices of orthonormal, but $\sigma_i$ are always the same. $\sigma_i$ are called the \cindex{singular value} of $T$.
\end{definition}
...
\begin{definition}[Pseudoinverse]
    Let $T: V \rightarrow W$ be a linear transformation, and $L: \nullspace{T}^\perp \rightarrow \rangespace{T}$. The \cindex{pseudoinverse} (\cindex{Moore-Penrose generalized inverse}) $\pseudoinverse{T}$ is defined as the unique linear transformation $\pseudoinverse{T}: W \rightarrow V$ that
    \begin{equation}
