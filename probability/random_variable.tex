...
\begin{definition}[sigma algebra]
    A collection of subsets of $S$ is called \cindex{sigma algebra} $\mathcal{B}$ if
    \begin{enumerate}
...
\begin{theorem}
    If $X$ is normally distributed with $\mu$ and $\sigma^2$, then $Y = aX + b$ is a normal distribution with $a\mu + b$ and $(a\sigma)^2$. So $Y =\displaystyle \frac{X - \mu}{\sigma}$ is normally distributed with $0$ and $1$, which is called \cindex{standard normal distribution}.
\end{theorem}
...
\begin{theorem}
    Gamma function is an extension of $n!$ to $\realnumber$.
    \begin{equation}
