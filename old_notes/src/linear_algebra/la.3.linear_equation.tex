...
\end{theorem}
\begin{proof}
    If $\exists x \neq 0 \mleft(x \in \nullspace{A^\top} \cap \rangespace{A} \mright)$. Then $(A^\top x = 0) \wedge \mleft(\exists y(x = A y) \mright)$. So $x^\top x = y^\top A^\top x = y^\top ( A^\top x) = 0 $ and then $x =0$. According to \thmref{rankoftwomatrix}, $\rank{A^\top A} = \rank{A^\top} - \dimension{\nullspace{A^\top} \cap \rangespace{A}} = \rank{A}$.
\end{proof}
